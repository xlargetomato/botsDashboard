...
CREATE INDEX IF NOT EXISTS idx_bot_status ON whatsapp_bots(status);
CREATE INDEX IF NOT EXISTS idx_bot_user ON whatsapp_bots(user_id);
CREATE INDEX IF NOT EXISTS idx_bot_subscription ON whatsapp_bots(subscription_id);
CREATE INDEX IF NOT EXISTS idx_bot_responses_trigger ON bot_responses(trigger);
CREATE INDEX IF NOT EXISTS idx_bot_block_rules_pattern ON bot_block_rules(pattern);

-- Add trigger for bot limit enforcement
DELIMITER //

DROP TRIGGER IF EXISTS after_bot_insert//
