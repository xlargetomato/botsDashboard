...
function ResetPasswordForm() {
  ...
  const [step, setStep] = useState(1); // 1: Password reset form, 2: Success message
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(isRtl 
        ? `فشل إعادة تعيين كلمة المرور: ${error.message}` 
              ...
              {step === 2 && (
                <div className="text-center py-8">
                  ...
                  <h2 className="text-3xl font-bold font-cairo text-gray-900 dark:text-white mb-4">
                    {isRtl ? 'تم إعادة تعيين كلمة المرور' : 'Password Reset Complete'}
                  </h2>
